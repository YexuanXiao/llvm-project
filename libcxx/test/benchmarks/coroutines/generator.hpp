// libc++ has not yet implemented std::generator; this implementation comes
// from https://github.com/llvm/llvm-project/pull/201854
// -*- C++ -*-
//===----------------------------------------------------------------------===//
//
...
template <class _Ref, class _Val>
using __gen_ref _LIBCPP_NODEBUG = conditional_t<is_void_v<_Val>, _Ref &&, _Ref>;
  ...
  struct __final_awaiter {
    ...
    await_suspend(coroutine_handle<_Promise> __current) noexcept {
      ...
      if (__current.promise().__is_recursive()) {
        ...
        // Transfers execution to its parent, which is the generator that yields
        // it
        return __parent;
      }
