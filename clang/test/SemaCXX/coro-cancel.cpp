...
// The builtins are usable directly on a frame handle with the promise
// alignof/sizeof.
void builtins(void *handle) {
  __builtin_coro_request_cancel(handle, 8, 24);
  bool b = __builtin_coro_cancel_requested(handle, 8, 24);
  (void)b;
}
