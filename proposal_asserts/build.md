...
| 区域 | 改动 |
|---|---|
| `llvm/` | **零新增 intrinsic**。仅 `CoroFrame` 一处：为**所有有 promise 的协程**（即所有 C++ 协程）在 promise 后无条件预留 1 字节取消标志（i8 header 字段）；LLVM 不关心 handler 是否存在。无 promise 的手写 IR 协程不添加，帧布局不变 |
| `clang/` | 新增 2 个 builtin `__builtin_coro_request_cancel` / `__builtin_coro_cancel_requested`（降低为既有 `llvm.coro.promise` + 指针运算）；`CoroutineBodyStmt` 增加 `OnCancellation` 子语句；Sema 查找 `promise.unhandled_cancellation()`（**可选**：未声明则不生成任何取消代码）；CodeGen 在 promise 构造后初始化标志、在每个挂起点恢复块（await_resume 之前）插入取消检查、生成取消块（`unhandled_cancellation` + final suspend） |
| `libcxx/` | `coroutine_handle<Promise>` 新增 `request_cancel()` / `cancel_requested()`（模板成员，调用 builtin 并传 `alignof/sizeof(Promise)`） |

设计要点：**控制流完全由前端管理**——每次 resume 在挂起点恢复块检查标志，
取消则跳转取消块（`unhandled_cancellation` + final suspend）；**LLVM 只提供
数据**（promise 后 1 字节标志，所有有 promise 的协程无条件），不关心
handler。destroy/cleanup 克隆中恢复点检查经恒假条件不可达，`instcombine`
折叠后自动删除。

ABI 兼容性：取消标志位于帧中 promise 对象之后（`frame + 2*ptrsize +
sizeof(Promise)`），resume/destroy 函数指针与 promise 投影偏移不变；帧总
大小向上取整到 `FrameAlign`，通常不增加。`request_cancel()` 的**语言前置
条件**是协程取消感知（promise 声明 `unhandled_cancellation`），对非取消
感知协程调用是 UB（[coroutine.handle.cancel]）；标志字节对所有有 promise
的协程无条件保留只是让违反前置条件的写入不越界——宽松实现，标准不承诺。
...
### requires 约束（`/tmp/coro_test/constraint_test.cpp`）

验证 `request_cancel()` / `cancel_requested()` 是**受约束成员**：仅当
promise 声明了 `unhandled_cancellation()` 时才存在。

```bash
# 第一段（-DOK，取消感知 promise）：应编译通过并运行返回 0
$CC -std=c++23 -O2 -DOK -nostdinc++ \
  -I /home/bizwen/llvm-project/libcxx/include \
  -I /tmp/coro_test/include \
  /tmp/coro_test/constraint_test.cpp -o /tmp/coro_test/constraint_ok \
  -L$LIB -lc++ -lc++abi -Wl,-rpath,$LIB

# 第二段（-DBAD，非取消感知 promise）：应编译失败
#   error: invalid reference to function 'request_cancel': constraints not satisfied
$CC -std=c++23 -DBAD -nostdinc++ \
  -I /home/bizwen/llvm-project/libcxx/include \
  -I /tmp/coro_test/include -fsyntax-only /tmp/coro_test/constraint_test.cpp
```
