...
//
// The symmetric transfer (tail call) carries the cancellation chain up to the
// root within a single client-side resume, so the chain completes without
// intermediate throws/catches and without exposing stale yielded values.
//
...
using __gen_val _LIBCPP_NODEBUG = conditional_t<is_void_v<_Val>, remove_cvref_t<_Ref>, _Val>;
...
#    if _LIBCPP_HAS_EXCEPTIONS
    if (__is_root()) {
      ...
      if (__root_data.__cancelled_child) {
        auto&& __source = __root_data.__cancelled_child.promise().__get_recursive_data();
...
private:
  using __rebind _LIBCPP_NODEBUG           = allocator_traits<_Allocator>::template rebind_alloc<__gen_aligned_block>;
  using __rebind_size_type _LIBCPP_NODEBUG = allocator_traits<__rebind>::size_type;
...
private:
  ...
  using __rref _LIBCPP_NODEBUG = conditional_t<is_lvalue_reference_v<_Ref>, remove_reference_t<_Ref>&&, _Ref>;
